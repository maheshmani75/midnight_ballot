...
| `api/` | Shared types and the `votingAPI` class used by both `cli/` and `web/` |
...
- Package manager is npm workspaces — use `npm run <script> -w <package-name>` (package names are
  `@midnight-ntwrk/voting-contract`, `@midnight-ntwrk/voting-api`, `@midnight-ntwrk/voting-cli`,
  `@midnight-ntwrk/voting-web`), not the folder path.
